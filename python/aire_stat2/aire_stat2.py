...
aire_stat2.py: envía osc a dos clientes
"""
...
def init_osc(osc_host = OSC_HOST, osc_port = OSC_PORT):
    ...
    return
def init_osc2(osc_host = OSC_HOST2, osc_port = OSC_PORT2):
    global OSC_CLIENT2
    OSC_CLIENT2 = OSCClient(osc_host, osc_port)
    return
def update_data_send(i=0):
    global contaminantes, fechas, OSC_CLIENT, OSC_CLIENT2, actual_set, actual_set_means
    print("\n\n[timetag]: ", fechas[i])
    ...
    for j,s in enumerate(substances):
        ...
        if sws[j]:
            ...
            if (modes[j]):
                ...
                OSC_CLIENT2.send_message(ruta, [aux_sam])
                print("{} \t{:0.3f}\t({})".format(s, aux_sam, e))
            else:
                OSC_CLIENT.send_message(ruta, [aux_mean])
                OSC_CLIENT2.send_message(ruta, [aux_mean])
